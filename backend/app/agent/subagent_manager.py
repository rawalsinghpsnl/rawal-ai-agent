...
async def spawn_subagent(
    *,
    ...
    if parent_ctx is not None and parent_ctx.depth >= MAX_SUBAGENT_DEPTH:
        raise ValueError("Subagent depth limit reached; do this work directly instead of spawning.")
    ...
    *,
    ...
    try:
        ...
        out = await run_subagent(
            ...
            tools_blocked=await _get_field(agent_id, "tools_blocked"),
        )
        # run_subagent returns (report, steps); tolerate a plain string from
        # any monkeypatched/legacy caller.
        if isinstance(out, tuple):
            report, steps_completed = out
        else:
            report, steps_completed = out, 0

        duration_ms = int((time.time() - started_at) * 1000)
        summary = report[:2000] + "..." if len(report) > 2000 else report

        # Persist the FULL report to a workspace file so the main agent (and
        # the user) can retrieve it without truncation. Best-effort: a DB
        # failure must never lose the in-memory report.
        report_path = ""
        try:
            report_path = f".agents/{agent_id}.md"
            await parent_ctx.sandbox.write_file(report_path, report)
        except Exception:
            report_path = ""
        ...
        async with SessionLocal() as db:
            ...
            if agent:
                ...
                agent.steps_completed = steps_completed if steps_completed > 0 else agent.max_steps
                meta = dict(agent.meta or {})
                if report_path:
                    meta["report_path"] = report_path
                meta["result_chars"] = len(report)
                agent.meta = meta
                await db.commit()
...
async def get_subagent_status(agent_id: str) -> dict[str, Any] | None:
    ...
    async with SessionLocal() as db:
        ...
        return {
            ...
            "result_chars": len(agent.result or ""),
            "report_path": meta.get("report_path", ""),
            "error": agent.error,
            "max_steps": agent.max_steps,
            "steps_completed": agent.steps_completed or 0,
...
async def wait_for_subagent(
    ...
    """Efficiently wait for one subagent to reach a terminal state.

    Single await — the caller (e.g. the `agent_wait` tool) blocks in ONE tool
    call instead of burning a main-loop step per poll. Returns the final
    status dict, or None if the agent vanished.
    """
    ...
    return await get_subagent_status(agent_id)
