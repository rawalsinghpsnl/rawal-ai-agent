...
async def run_subagent(
    *,
    ...
    Returns (final_report, steps_completed). Enforces `tools_blocked` so a
    read-only specialist (explorer/researcher/...) can never mutate files even
    if the model tries. Callers that only need the text can use index [0].
    """
    async with SessionLocal() as db:
    ...
    blocked = set(tools_blocked or [])
    client = resolved.client()
    all_schemas = registry.schemas(exclude={"task"})
    tools = [s for s in all_schemas if s["function"]["name"] not in blocked]
    system = build_system_prompt(
        ...
        "IMPORTANT — no truncation: your final message IS the deliverable. Include the "
        "complete result (full code, full research notes, all file paths you created). "
        "If you created files, list every path. Do not summarise away details."
    )
    ...
    for step in range(max_steps):
        ...
        for call in completion.tool_calls:
            ...
            elif call.name in blocked:
                output = (
                    f"ERROR: tool `{call.name}` is blocked for this subagent. "
                    "Do the work with your allowed tools or report back without it."
                )
            else:
    ...
    # NOTE: parent.state is shared between parallel subagents, so never use a
    # single shared key here (last-writer-wins race). Steps are returned
    # directly; managers persist them per-agent in the DB.
    return (final_text or "(subagent produced no final report)", steps_completed)
