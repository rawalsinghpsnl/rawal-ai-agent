...
Implements the Agent Skills spec subset (`SKILL.md` packs):
`<slug>/SKILL.md` with YAML frontmatter (`name`,
...
def discover_skills(workspace: str) -> list[InstalledSkill]:
    ...
    # Skill directories: <slug>/SKILL.md (nested up to MAX_DEPTH).
...
def _github_headers() -> dict[str, str]:
    headers = {"Accept": "application/vnd.github+json", "User-Agent": "RawalAiAgent/2.0"}
...
def _fetch_text(url: str, limit: int = FETCH_LIMIT_BYTES) -> str:
    ...
    try:
        ...
        with httpx.Client(timeout=30, follow_redirects=False) as client:
            for _ in range(6):
                with client.stream("GET", current, headers={"User-Agent": "RawalAiAgent/2.0"}) as response:
...
def install_from_zip_url(workspace: str, url: str) -> list[InstalledSkill]:
    ...
    try:
        ...
        with httpx.Client(timeout=60, follow_redirects=False) as client:
            for _ in range(6):
                response = client.get(current, headers={"User-Agent": "RawalAiAgent/2.0"})
                if response.status_code in (301, 302, 303, 307, 308):
                    location = response.headers.get("location", "")
                    if not location:
                        raise AppError("Redirect without location", code="fetch_failed")
...
def _install_from_zip_bytes(workspace: str, data: bytes, origin: str, origin_url: str = "") -> list[InstalledSkill]:
    ...
    with tempfile.TemporaryDirectory(prefix="rawal-skill-") as tmp:
        tmp_path = Path(tmp)
        try:
...
def marketplace_live(query: str = "") -> list[dict[str, Any]]:
    """Curated catalog merged with the community skill library."""
    ...
    # Community library (kept separate via source badge).
    try:
