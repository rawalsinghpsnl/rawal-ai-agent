...
REMOTE_ROOT = f"{REMOTE_HOME}/rawal"
SYNC_MAX_FILES = 200
...
class SuperserveClient:
    ...
    def _control_headers(self) -> dict[str, str]:
        return {"X-API-Key": self.api_key, "User-Agent": "RawalAiAgent/2.0"}

    def _data_headers(self, access_token: str, sandbox_id: str = "") -> dict[str, str]:
        headers = {"X-Access-Token": access_token, "User-Agent": "RawalAiAgent/2.0"}
        if sandbox_id:
...
class SuperserveSandbox(Sandbox):
    ...
    Remote paths live under /home/user/rawal; `workdir` reports the local
    ...
    async def start(self) -> SandboxInfo:
        if self._box_id:
            ...
            created = await self._client.create(
                f"rawal-{self._thread_id[:16]}",
                template=settings.SUPERSERVE_TEMPLATE,
                metadata={"thread": self._thread_id, "app": "rawal-ai-agent"},
            )
    ...
    async def _make_one(self) -> PooledBox | None:
        try:
            created = await self.client.create(
                f"rawal-pool-{int(time.time()) % 100000}",
                template=self.template or settings.SUPERSERVE_TEMPLATE,
                metadata={"app": "rawal-ai-agent", "pool": "warm"},
            )
