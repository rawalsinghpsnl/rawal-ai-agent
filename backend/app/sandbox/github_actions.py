  ...
  run-sandbox:
    ...
    steps:
      ...
      - name: Execute and capture
        env:
          RAWAL_COMMAND: ${{ inputs.command }}
          RAWAL_RUN_ID: ${{ inputs.run_id }}
        run: |
          set +e
          bash -c "$RAWAL_COMMAND" > /tmp/rawal-stdout.txt 2> /tmp/rawal-stderr.txt
          code=$?
          python3 - "$code" <<'EOF'
          import json, sys
          code = int(sys.argv[1])
          with open('/tmp/rawal-stdout.txt', encoding='utf-8', errors='replace') as f:
              stdout = f.read()[-400000:]
          with open('/tmp/rawal-stderr.txt', encoding='utf-8', errors='replace') as f:
              stderr = f.read()[-100000:]
          with open('rawal-result.json', 'w', encoding='utf-8') as f:
      ...
      - name: Upload result
        ...
        with:
          name: rawal-result-${{ inputs.run_id }}
          path: rawal-result.json
          retention-days: 1
"""
...
def parse_result_artifact(zip_bytes: bytes) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        ...
        for name in zf.namelist():
            if name.rsplit("/", 1)[-1] == "rawal-result.json":
                import json as json_lib

                data = json_lib.loads(zf.read(name).decode("utf-8"))
                if isinstance(data, dict):
                    return data
    raise ValueError("rawal-result.json not found in artifact")
...
class GitHubActionsSandbox(Sandbox):
    ...
    async def _download_result(self, run: dict[str, Any]) -> dict[str, Any]:
        ...
        artifact = next(
            (a for a in artifacts if isinstance(a, dict) and a.get("name", "").startswith("rawal-result-")),
            None,
        )
        if artifact is None:
