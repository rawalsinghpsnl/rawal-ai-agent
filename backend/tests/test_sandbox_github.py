...
def _zip_result(payload: dict) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w") as zf:
        zf.writestr("rawal-result-abc123/rawal-result.json", json.dumps(payload))
    return buf.getvalue()
    ...
    async def request(self, method, path, **kwargs):
        ...
        if "/actions/workflows/" in path and path.endswith("/runs"):
            return {"workflow_runs": [{
                "id": 99, "display_title": f"rawal-{self.last_run_id}",
                "status": "completed", "conclusion": "success",
            }]}
        if "/actions/runs/" in path and path.endswith("/artifacts"):
            return {"artifacts": [{"id": 7, "name": "rawal-result-abc123"}]}
...
def test_find_run_matches_marker():
    runs = [
        ...
        {"id": 2, "display_title": "rawal-abc123", "status": "in_progress"},
    ]
...
def test_workflow_yaml_has_result_channel():
    assert 'run-name: "rawal-${{ inputs.run_id }}"' in WORKFLOW_YAML
    assert "upload-artifact" in WORKFLOW_YAML
    assert "rawal-result.json" in WORKFLOW_YAML
