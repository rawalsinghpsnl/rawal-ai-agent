...
// rawal-ai-agent rebrand: migrate legacy bhati.* keys once so existing installs keep data.
try {
  migrateLegacyKeys([
    "outbox",
    "keymap",
...
const OUTBOX_KEY = "rawal.outbox";
const KEYMAP_KEY = "rawal.keymap";
...
async function sha256(text: string): Promise<string> {
  // rawal-ai-agent rebrand: accept both new and legacy PIN salts so existing locks keep working.
  const bytes = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(`rawal-pin:${text}`));
  return [...new Uint8Array(bytes)].map((b) => b.toString(16).padStart(2, "0")).join("");
}

async function sha256Legacy(text: string): Promise<string> {
...
export async function verifyPin(pin: string, storedHash: string): Promise<boolean> {
  ...
  return (await sha256Legacy(pin)) === storedHash;
}
...
export const useStore = create<State>((set, get) => ({
  ...
  sandbox: null,
  followAgent: true,
  voiceEnabled: localStorage.getItem('rawal.voice') === 'true',
  voiceMode: (localStorage.getItem('rawal.voiceMode') as any) || 'send',
  voiceTrigger: localStorage.getItem('rawal.voiceTrigger') || 'send now',
  voiceRate: Number(localStorage.getItem('rawal.voiceRate')) || 1,
  voicePitch: Number(localStorage.getItem('rawal.voicePitch')) || 1,
  wakeEnabled: localStorage.getItem('rawal.wake') === '1',
  translateMode: localStorage.getItem('rawal.translateMode') === '1',
  translateTarget: (localStorage.getItem('rawal.translateTarget') as "en" | "hi") || "en",
  callActive: false,
  lowPerf: localStorage.getItem('rawal.lowperf') === '1',
  deviceMode: ((localStorage.getItem('rawal.deviceMode') as any) || 'auto') as "auto" | "mobile" | "desktop",
  effectiveDeviceMode: (() => {
    const saved = typeof window !== "undefined" ? localStorage.getItem("rawal.deviceMode") : null;
    if (saved === "mobile" || saved === "desktop") return saved;
    return typeof window !== "undefined" && window.innerWidth < 768 ? "mobile" : "desktop";
  })(),
  networkMode: ((localStorage.getItem('rawal.networkMode') as any) || 'auto') as "auto" | "ultra_low" | "high_speed",
  effectiveNetworkMode: localStorage.getItem("rawal.networkMode") === "ultra_low" ? "ultra_low" : "high_speed",
  networkStats: {
    effectiveType: "4g",
    downlink: 10,
    rtt: 50,
  ...
  outbox: loadOutbox(),
  pinnedIds: [],
  notifyEnabled: localStorage.getItem('rawal.notify') === '1',
  accentHue: (() => {
    const raw = localStorage.getItem('rawal.accentHue');
    const n = raw === null ? null : Number(raw);
    return n === null || Number.isNaN(n) ? null : n;
  })(),
  keymap: loadKeymap(),
  lockHash: localStorage.getItem('rawal.pinHash'),
  unlocked: sessionStorage.getItem('rawal.unlocked') === '1',
  installEvt: null,
  ...
  async bootstrap() {
    ...
    const lastProject = localStorage.getItem("rawal.project");
  ...
  async selectProject(id) {
    ...
    localStorage.setItem("rawal.project", id);
    const threads = await api.listThreads(id);
    set({ threads });
    const lastThread = localStorage.getItem(`rawal.thread.${id}`);
  ...
  async selectThread(id) {
    ...
    if (projectId) localStorage.setItem(`rawal.thread.${projectId}`, id);
    try {
      const raw = localStorage.getItem(`rawal.pins.${id}`);
  ...
  async deleteProject(id) {
    ...
    if (get().activeProjectId === id) {
      localStorage.removeItem("rawal.project");
  ...
  setLowPerf: (lowPerf) => {
    try {
      localStorage.setItem("rawal.lowperf", lowPerf ? "1" : "0");
    } catch {
      /* storage unavailable */
    }
  ...
  setDeviceMode: (deviceMode) => {
    try {
      localStorage.setItem("rawal.deviceMode", deviceMode);
    } catch {
      /* storage unavailable */
    }
  ...
  setNetworkMode: (networkMode) => {
    try {
      localStorage.setItem("rawal.networkMode", networkMode);
    } catch {
      /* storage unavailable */
    }
  ...
  setWakeEnabled: (wakeEnabled) => {
    try {
      localStorage.setItem("rawal.wake", wakeEnabled ? "1" : "0");
    } catch {
      /* ignore */
    }
    set({ wakeEnabled });
  ...
  setVoiceSettings: (settings) => {
    set(s => {
      if (settings.voiceEnabled !== undefined) localStorage.setItem('rawal.voice', String(settings.voiceEnabled));
      if (settings.voiceMode) localStorage.setItem('rawal.voiceMode', settings.voiceMode);
      if (settings.voiceTrigger) localStorage.setItem('rawal.voiceTrigger', settings.voiceTrigger);
      if (settings.voiceRate !== undefined) localStorage.setItem('rawal.voiceRate', String(settings.voiceRate));
      if (settings.voicePitch !== undefined) localStorage.setItem('rawal.voicePitch', String(settings.voicePitch));
      if (settings.translateMode !== undefined) localStorage.setItem('rawal.translateMode', settings.translateMode ? '1' : '0');
      if (settings.translateTarget) localStorage.setItem('rawal.translateTarget', settings.translateTarget);
      return { ...s, ...settings };
    });
  },
  ...
  togglePin: (messageId) => {
    ...
    if (id) {
      try {
        localStorage.setItem(`rawal.pins.${id}`, JSON.stringify(pinnedIds));
      } catch {
        /* ignore */
      }
    }
  ...
  setNotifyEnabled: (notifyEnabled) => {
    try {
      localStorage.setItem("rawal.notify", notifyEnabled ? "1" : "0");
    } catch {
      /* ignore */
    }
  ...
  setAccentHue: (accentHue) => {
    try {
      if (accentHue === null) localStorage.removeItem("rawal.accentHue");
      else localStorage.setItem("rawal.accentHue", String(accentHue));
    } catch {
      /* ignore */
    }
  ...
  async setPin(pin) {
    if (!pin) {
      try {
        localStorage.removeItem("rawal.pinHash");
      } catch {
        /* ignore */
      }
    ...
    try {
      localStorage.setItem("rawal.pinHash", lockHash);
      sessionStorage.setItem("rawal.unlocked", "1");
    } catch {
      /* ignore */
    }
  ...
  lockNow: () => {
    try {
      sessionStorage.removeItem("rawal.unlocked");
    } catch {
      /* ignore */
    }
    set({ unlocked: false });
